...
class _ChatScreenState extends State<ChatScreen> {
    ...
    await _speech.listen(
      onResult: (result) {
        ...
        if (result.finalResult) {
          ...
          if (_handsFreeMode &&
              result.recognizedWords.toLowerCase().startsWith("hey securai")) {
    ...
    setState(() {
      ...
      if (_handsFreeMode) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Hands-Free Mode Enabled. Say "Hey SecurAI..."'),
          ),
        );
      } else {
        ...
        _stopListening();
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Hands-Free Mode Disabled.')),
        );
      }
    });
  }
  ...
  void _pickImage() async {
    ...
    if (image != null) {
      ...
      setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Image Attached for Vision AI!')),
        );
      });
    }
  }
  ...
  void _pickFile() async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      setState(() {
        _controller.text =
            "Please analyze this file: ${result.files.single.name}\n\n```\n$contents\n```\n";
      });
    }
  }
      ...
      setState(() {
        _messages = response
            .map<Map<String, String>>(
              (row) => {"sender": row['role'], "text": row['content']},
            )
            .toList();
      });
      _scrollToBottom();
  ...
  void _sendMessage([String? text]) async {
    ...
    if (widget.threadId == "default") {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please start a New Chat from the menu first!"),
        ),
      );
      return;
    }
    ...
    final stream = _apiService.streamMessage(
      userMsg,
      persona,
      language,
      imageBase64: b64,
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildStatusRow(
                    'System Health',
                    '${(metrics['system_health'] ?? 0).toStringAsFixed(1)}%',
                    Icons.favorite,
                    Colors.greenAccent,
                  ),
                  ...
                  _buildStatusRow(
                    'CPU Usage',
                    '${(metrics['cpu_percent'] ?? 0).toStringAsFixed(1)}%',
                    Icons.memory,
                    Colors.orangeAccent,
                  ),
                  const SizedBox(height: 16),
                  _buildStatusRow(
                    'Memory Usage',
                    '${(metrics['ram_percent'] ?? 0).toStringAsFixed(1)}%',
                    Icons.storage,
                    Colors.purpleAccent,
                  ),
                ],
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('AI response ready to share!'),
                          ),
                        );
                      },
                      tooltip: "Share",
