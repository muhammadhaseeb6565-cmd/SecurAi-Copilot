...
class _ReportsScreenState extends State<ReportsScreen> {
        ...
        content: SizedBox(
          ...
          child: Markdown(
            ...
            styleSheet: MarkdownStyleSheet(
              code: TextStyle(backgroundColor: Colors.black.withValues(alpha: 0.2), fontFamily: 'monospace'),
            ),
          ),
        ),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Real-Time Security Scans', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        elevation: 0,
      ...
      body: Column(
        children: [
          ...
          if (snapshot.hasError || !snapshot.hasData) {
            return Center(child: Text('Error loading alerts.', style: TextStyle(color: Theme.of(context).colorScheme.error)));
          }
          ...
          return RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final isHigh = alert["severity"].toString().toLowerCase() == "high";
                final isLow = alert["severity"].toString().toLowerCase() == "low";
                
                Color statusColor = isHigh ? Colors.redAccent : (isLow ? Colors.greenAccent : Colors.orangeAccent);
                    ...
                    child: Column(
                      ...
                      children: [
                          ...
                          children: [
                            Expanded(
                              child: Row(
                                children: [
                                  Icon(
                                    isHigh ? Icons.error_outline : (isLow ? Icons.check_circle_outline : Icons.warning_amber),
                                    color: statusColor,
                                  ),
                            ...
                            Container(
                              ...
                              child: Text(alert["severity"]!, style: TextStyle(color: statusColor, fontSize: 10, fontWeight: FontWeight.bold)),
                            ),
                          ],
                        ),
                        const SizedBox(height: 16),
                        Container(
                          ...
                          child: Text(alert["details"]!, style: TextStyle(fontFamily: 'monospace', fontSize: 12, color: Theme.of(context).colorScheme.onSurface)),
                        ),
                        const SizedBox(height: 16),
                        if (!isLow)
                          Wrap(
